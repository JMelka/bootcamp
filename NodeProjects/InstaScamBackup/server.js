...
app.post('/insertPictureAndComment/', multer({dest: './public/photos/'}).single('dropzone'), function (req, res) {
    ...
    insertPic.then((val) => {
        if (message.length > 0){
            var insertPic = db.insertComment(val[0].pictureId, picture.userName, picture.message);
        }
    }).then((val) => {
        res.send('Picture for ' + user.userName + ' is added successfully!');
    }).catch(
...
app.get('/getPictureComments/:pictureId', function (req, res) {
